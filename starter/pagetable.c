...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		//check if it is dirty (swapout if it is dirty).
		if(victim->frame & PG_DIRTY){
			int new_swap_offset;
			new_swap_offset = swap_pageout(frame, victim->swap_off);

			// check if it is a vaild swap
			if(new_swap_offset == INVALID_SWAP){

                exit(1);
            }
            
            // update the victim's swap offset
...
char *find_physpage(addr_t vaddr, char type) {
  ...
  // Check if p is valid or not, on swap or not, and handle appropriately
  	if (!(p->frame & PG_VALID) && !(p->frame & PG_ONSWAP)) 
  	{
    	...
    	// invalid and not on swap, then this is the first reference 
		miss_count++;
